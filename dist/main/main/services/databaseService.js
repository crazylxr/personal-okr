...
class DatabaseService {
    ...
    async createTask(task) {
        const result = await database_1.database.run(`INSERT INTO tasks (okr_id, title, description, estimated_hours, actual_hours, status, updated_at) 
       VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)`, [task.okr_id, task.title, task.description, task.estimated_hours, task.actual_hours, task.status]);
        return result.lastID;
    }
