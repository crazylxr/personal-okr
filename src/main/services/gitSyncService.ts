...
import { GitConfig, SyncData, GitSyncStatus } from '../../types/database';
...
class MainGitSyncService {
  ...
  constructor() {
    this.repoPath = join(app.getPath('userData'), 'git-sync');
    this.dataDir = join(this.repoPath, 'data');
  }

  /**
   * 创建远程仓库
   */
  public async createRemoteRepository(config: GitConfig): Promise<string> {
    ...
    try {
      ...
      throw new Error(`创建远程仓库失败: ${(error as Error).message}`);
    }
  }
   ...
   * 检查 GitHub 仓库是否存在
   */
  private async checkGitHubRepositoryExists(token: string, name: string): Promise<string | null> {
        ...
        res.on('end', () => {
          try {
            if (res.statusCode === 200) {
              ...
              console.log(`检查 GitHub 仓库是否存在的响应: ${JSON.stringify(repos.map((e: any) => e.name))}`);
              const existingRepo = repos.find((repo: any) => repo.name === name);
              if (existingRepo) {
   ...
   * 检查 GitLab 仓库是否存在
   */
  private async checkGitLabRepositoryExists(token: string, name: string): Promise<string | null> {
    return new Promise((resolve, reject) => {
      const options: any = {
        hostname: 'gitlab.com',
        port: 443,
        ...
        res.on('end', () => {
          try {
            if (res.statusCode === 200) {
              ...
              const existingProject = projects.find((project: any) => project.name === name);
              if (existingProject) {
   ...
   * 创建 GitHub 仓库
   */
  private async createGitHubRepository(token: string, name: string, description: string, isPrivate: boolean): Promise<string> {
    return new Promise((resolve, reject) => {
   ...
   * 创建 GitLab 仓库
   */
  private async createGitLabRepository(token: string, name: string, description: string, isPrivate: boolean): Promise<string> {
    return new Promise((resolve, reject) => {
      const data = JSON.stringify({
        name,
        description,
   ...
   * 初始化 Git 仓库和配置
   */
  public async initializeRepository(config: GitConfig): Promise<void> {
    try {
      ...
      throw new Error(`Git 仓库初始化失败: ${(error as Error).message}`);
    }
  }
  ...
  private async setupAuthentication(): Promise<void> {
    if (!this.config || !this.git) return;
    ...
    try {
      ...
      if (this.config.authMethod === 'token' && this.config.credentials.token) {
        ...
        if (cleanUrl.startsWith('git@')) {
          ...
          remoteUrl = cleanUrl.replace(
            'https://',
            `https://${this.config.credentials.token}@`
          );
        }
        ...
        // 使用用户名密码
        const remoteUrl = cleanUrl.replace(
          'https://',
          `https://${this.config.credentials.username}:${this.config.credentials.password}@`
        );
        await this.git.removeRemote('origin').catch(() => {});
        await this.git.addRemote('origin', remoteUrl);
   ...
   * 尝试不同的连接协议
   */
  private async tryDifferentProtocols(operation: () => Promise<void>): Promise<void> {
    if (!this.config) throw new Error('Git 配置未初始化');
    ...
    for (const protocol of protocols) {
      try {
        ...
        this.config.authMethod = protocol as any;
        await this.setupAuthentication();
        await operation();
        console.log(`${protocol} 协议连接成功`);
        return; // 成功则返回
      } catch (error) {
        console.log(`${protocol} 协议失败:`, (error as Error).message);
  ...
  public async exportData(): Promise<void> {
    ...
    try {
      ...
      throw new Error(`数据导出失败: ${(error as Error).message}`);
    }
  }
  ...
  public async commitChanges(message?: string): Promise<void> {
    ...
    try {
      ...
      throw new Error(`提交更改失败: ${(error as Error).message}`);
    }
  }
      ...
      try {
        ...
        if (isConnectionResetError && !protocolSwitched && attempt < maxRetries) {
          ...
          try {
            ...
            console.log('协议切换失败:', (protocolError as Error).message);
   ...
   * 完整同步流程
   */
  public async syncData(): Promise<SyncData> {
    if (!this.git || !this.config) {
      throw new Error('Git 未初始化');
    }
    ...
    try {
      ...
      throw new Error(`Git 同步失败: ${(error as Error).message}`);
    }
  }
   ...
   * 获取同步状态
   */
  public async getStatus(): Promise<GitSyncStatus> {
    try {
      if (!this.git) {
        return {
          isInitialized: false,
   ...
   * 测试代理连接
   */
  public async testProxyConnection(proxyConfig: NonNullable<GitConfig['proxy']>): Promise<boolean> {
    try {
   ...
   */
  private async testGitHubRepositoryConnection(owner: string, repo: string, token: string): Promise<boolean> {
    return new Promise((resolve, reject) => {
      const options: any = {
        hostname: 'api.github.com',
        port: 443,
   ...
   */
  private extractOwnerAndRepo(remoteUrl: string): { owner: string; repo: string } | null {
    try {
      // 支持多种 GitHub URL 格式
      const patterns = [
   ...
   * 更新配置
   */
  public updateConfig(newConfig: GitConfig): void {
   ...
   * 设置代理环境变量
   */
  private async setupProxyEnvironment(): Promise<void> {
    if (!this.config?.proxy?.enabled) {
      return;
    }
   ...
   * 构建代理 URL
   */
  private buildProxyUrl(proxy: NonNullable<GitConfig['proxy']>): string {
...
export const mainGitSyncService = new MainGitSyncService();
export default mainGitSyncService;