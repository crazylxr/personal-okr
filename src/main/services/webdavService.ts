...
class MainWebDAVService {
  ...
  private readonly SYNC_FILE_PATH = '/okr-manager-data.json';

  public async initializeClient(config: WebDAVConfig): Promise<void> {
    try {
  ...
  public async downloadData(): Promise<SyncData | null> {
    ...
    try {
      ...
      const data = JSON.parse(fileContent) as SyncData;
  ...
  private async syncRemoteToLocal(remoteData: SyncData): Promise<void> {
    try {
      // 这里应该实现更复杂的同步逻辑
      // 暂时简化为直接替换本地数据
...
export const mainWebDAVService = new MainWebDAVService();
export default mainWebDAVService;